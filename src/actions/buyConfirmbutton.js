...
export default {
  ...
  execute: async (interaction) => {
    ...
    if (currentAction.type !== "buyConfirming") return;

    const accepted = interaction.customId.split("#")[1] === "1";

    const events = [];
    if (accepted) await handleBuyConfirm(state, player, events);
    else await handleBuyCancel(state, player, events);
